...
void al5_group_unbind_user_without_group_spinlock(struct al5_group *group,
...
static int try_to_deliver_to_user(struct al5_group *group, struct al5_mail *mail)
{
	...
	if (user == NULL) {
		print_cannot_retrieve_user(group->device,
					   al5_mail_get_uid(mail));
		goto unlock;
	}
...
static void handle_mail(struct al5_group *group, struct al5_mail *mail)
{
	...
	if (error == AL_NO_USER) {
		if (should_destroy_channel_on_bad_feedback(mail_uid))
			destroy_orphan_channel(group,
					       al5_mail_get_chan_uid(mail));
		al5_free_mail(mail);
