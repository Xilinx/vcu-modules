...
/* return -ENOMEM if we failed to deliver, 0 otherwise */
		...
		while (mail) {
			...
			if (mail_uid == AL_MCU_MSG_CREATE_CHANNEL) {
				...
				while (ret != 0 && tries < 10000) {
					ret = al5_user_destroy_channel(user,
								       quiet);
					++tries;
				}

				if (ret != 0)
					printk("New issue: Failed to destroy channel on mcu. Something went wrong (%d)",
					       ret);
			}
...
int al5_user_destroy_channel(struct al5_user *user, int quiet)
{
	...
	if (quiet) {
		err = al5_check_and_send(
			user, create_quiet_destroy_channel_msg(user->chan_uid));
		if (err)
			goto unlock_mutexes;
	} else {
		err = al5_check_and_send(
			user, create_destroy_channel_msg(user->chan_uid));
		if (err)
			goto unlock_mutexes;
