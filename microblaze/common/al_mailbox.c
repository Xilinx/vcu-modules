...
static int not_enough_space_in_mailbox(size_t mailbox_size, size_t used_size,
				       size_t data_size)
{
...
static int mailbox_is_wrapping(size_t mailbox_size, unsigned int tail_value,
			       size_t data_size)
{
...
static void write_in_mailbox_until_wrap(struct mailbox *box,
					size_t *memcpy_size, unsigned int *tail,
					u8 **data)
{
...
/* Require mailbox_is_wrapping */
static void read_in_mailbox_until_wrap(struct mailbox *box, size_t *memcpy_size,
				       unsigned int *head, u8 **data)
{
...
static void write_data(struct mailbox *box, u8 *in_data, size_t size)
{
	...
	if (mailbox_is_wrapping(box->size, tail_value, size))
		write_in_mailbox_until_wrap(box, &size, &tail_value, &in_data);
...
int al5_mailbox_write(struct mailbox *box, struct al5_mail *mail)
{
	...
	size_t used_size = (tail_value >= head_value) ?
