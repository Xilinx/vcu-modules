...
static int request_all_firmwares(struct al5_codec_desc *codec,
				 const struct firmware **fw,
				 const struct firmware **bl_fw, char *fw_file,
				 char *bl_fw_file)
{
	int err;
	...
	if (err) {
		al5_err("bootloader firmware file '%s' not found", bl_fw_file);
		    ...
		    bool plugin_enabled)
{
	...
	init_msg.addr_v = (codec->suballoc_buf->dma_handle -
...
{
	struct al5_filp_data *private_data =
		kzalloc(sizeof(*private_data), GFP_KERNEL);
...
static int al5_setup_dma(struct al5_codec_desc *codec)
{
	...
	if (np) {
		...
		if (err) {
			dev_err(codec->device,
				"Couldn't get reserved memory (errno: %d)",
				err);
			goto out;
		}
		     ...
		     size_t max_users_nb)
{
...
#else
	codec->regs = devm_ioremap_nocache(&pdev->dev, res->start,
					   resource_size(res));
#endif
	...
	if (mem_node) {
		...
		if (!err) {
			...
			if (err)
				...
				err = add_memory(nid, mem_res.start,
						 resource_size(&mem_res) +
							 pgtable_padding,
						 MHP_NONE);
	...
	err = devm_request_threaded_irq(codec->device, irq, al5_hardirq_handler,
					al5_irq_handler, IRQF_SHARED,
					device_name, codec);
	if (err) {
		dev_err(&pdev->dev, "Failed to request IRQ #%d -> :%d", irq,
			err);
