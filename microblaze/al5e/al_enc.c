...
static int ioctl_usage(struct al5_user *user, unsigned int cmd)
{
	...
	pr_err("GET_DMA32_FD:%.8lX\n", (unsigned long)GET_DMA32_FD);

	return -EINVAL;
}

static long al5e_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case AL_MCU_WAIT_FOR_STATUS:
		...
		if (copy_from_user(&params, (void *)arg, sizeof(params)))
			return -EFAULT;
		ret = al5e_user_wait_for_status(user, &params);
...
static int al5e_setup_codec_cdev(struct al5_codec_desc *codec, int minor)
{
	...
	int err = al5_setup_cdev(&codec->cdev, &al5e_fops, THIS_MODULE,
				 al5e_codec_major, minor);

	if (err)
		return err;

	of_property_read_string(codec->device->of_node, "al,devicename",
				&device_name);
...
static int al5e_probe(struct platform_device *pdev)
{
	...
	codec = devm_kzalloc(&pdev->dev, sizeof(struct al5_codec_desc),
			     GFP_KERNEL);
	if (codec == NULL)
		return -ENOMEM;
...
static void al5e_remove(struct platform_device *pdev)
#else
...
{
	return al5_setup_chrdev_region(&al5e_codec_major, 0, al5e_codec_nr_devs,
				       "al5e");
}
