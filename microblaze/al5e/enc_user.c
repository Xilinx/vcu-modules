...
static struct al5_mail *
create_mail_from_bufpool(u32 msg_uid, u32 chan_uid,
				    ...
				    struct al5_channel_buffers buffers)
{
	...
	buffers.rec_buffers_size +=
		pagesize - (buffers.rec_buffers_size % pagesize);
				 ...
				 struct al5e_feedback_channel *fb_message)
{
	...
	if (msg->rc_plugin_fd >= 0) {
		err = al5_get_dmabuf_info(user->device, msg->rc_plugin_fd,
					  &rc_plugin_info);
		if (err)
			return err;
		mcu_rc_plugin_addr =
			al5_mcu_get_virtual_address(rc_plugin_info.bus_address);
		mcu_rc_plugin_size = rc_plugin_info.size;
	}

	err = al5_check_and_send(
		user, al5e_create_channel_param_msg(user->uid, param,
						    mcu_rc_plugin_addr,
						    mcu_rc_plugin_size));
			     ...
			     struct al5_config_channel *msg)
{
	...
	if (al5_chan_is_created(user)) {
		err = -EPERM;
		dev_err(user->device, "Channel already created (%d, %d)",
	...
	if (user->checkpoint == CHECKPOINT_ALLOCATE_BUFFERS) {
		...
		if (err) {
			dev_warn_ratelimited(
				user->device,
				"Failed internal buffers allocation, channel (%d, %d) isn't fully created yet",
				user->uid, user->chan_uid);
			goto fail_allocate;
		}
	...
	if (user->checkpoint == CHECKPOINT_SEND_INTERMEDIATE_BUFFERS) {
		...
		if (err) {
			dev_warn_ratelimited(
				user->device,
				"Failed to send intermediate buffers, channel (%d, %d) isn't fully created yet",
				user->uid, user->chan_uid);
			goto fail;
		}
	...
	if (user->checkpoint == CHECKPOINT_SEND_REFERENCE_BUFFERS) {
		...
		if (err) {
			dev_warn_ratelimited(
				user->device,
				"Failed to send reference buffers, channel (%d, %d) isn't fully created yet",
				user->uid, user->chan_uid);
			goto fail;
		}
	...
	dev_dbg(user->device, "Created channel (%d, %d)", user->uid,
		user->chan_uid);
			       ...
			       struct al5_encode_msg *msg)
{
	...
	err = al5_check_and_send(
		user, al5e_create_encode_one_frame_msg(user->chan_uid, msg));
...
int al5e_user_wait_for_status(struct al5_user *user, struct al5_params *msg)
{
	...
	if (!al5_chan_is_created(user)) {
		dev_dbg(user->device,
			"Cannot get a frame status if the channel isn't configured (%d, %d)",
			user->uid, user->chan_uid);
		err = -EPERM;
		goto unlock;
	}
	...
	if (user->non_block) {
		feedback = al5_queue_pop_no_wait(
			&user->queues[AL5_USER_MAIL_STATUS]);
		if (feedback)
				...
				struct al5_buffer *buffer)
{
	...
	if (!al5_chan_is_created(user)) {
		dev_err(user->device,
			"Cannot put a stream buffer if the channel isn't configured (%d, %d)",
			user->uid, user->chan_uid);
		err = -EPERM;
		goto unlock;
	}
	...
	if (buffer->external_mv_handle >= 0) {
		err = al5_get_dmabuf_info(user->device,
					  buffer->external_mv_handle,
	...
	stream_mcu_vaddr =
		al5_mcu_get_virtual_address(stream_buffer_info.bus_address);
...
int al5e_user_get_rec(struct al5_user *user, struct al5_reconstructed_info *msg)
{
	...
	if (!al5_chan_is_created(user)) {
		dev_err(user->device,
			"Cannot get a reconstructed buffer if the channel isn't configured (%d, %d)",
			user->uid, user->chan_uid);
		err = -EPERM;
		goto unlock;
	}
...
int al5e_user_release_rec(struct al5_user *user, s32 fd)
{
	...
	if (!al5_chan_is_created(user)) {
		dev_err(user->device,
			"Cannot release a reconstructed buffer if the channel isn't configured (%d, %d)",
			user->uid, user->chan_uid);
		err = -EPERM;
		goto unlock;
	}
...
int al5e_user_get(struct al5_user *user, struct al5_params *msg)
{
	...
	memcpy(msg->opaque, al5_mail_get_body(feedback) + sizeof(user->uid),
	       msg->size);
	al5_free_mail(feedback);
	mutex_unlock(&user->locks[AL5_USER_CHANNEL]);
	return 0;
