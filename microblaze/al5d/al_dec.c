...
static long al5d_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
{
	return al5_setup_chrdev_region(&al5d_codec_major, 0, al5d_codec_nr_devs,
				       "al5d");
}
...
static int create_module_class(void)
{
	...
	module_class = class_create("allegro_decode_class");
#else
