			     ...
			     struct al5_channel_config *msg)
{
	...
	err = al5_check_and_send(
		user, al5d_create_channel_param_msg(user->uid, &msg->param));
	if (err)
		goto unlock;
			       ...
			       struct al5_decode_msg *msg)
{
	...
	err = al5_check_and_send(
		user, al5d_create_decode_one_slice_msg(user->chan_uid, msg));
			       ...
			       struct al5_decode_msg *msg)
{
	...
	err = al5_check_and_send(
		user, al5d_create_decode_one_frame_msg(user->chan_uid, msg));
				  ...
				  struct al5_scstatus *msg)
{
	...
	if (user->non_block) {
		feedback =
			al5_queue_pop_no_wait(&user->queues[AL5_USER_MAIL_SC]);
		if (feedback)
