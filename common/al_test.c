...
/*
 * headers are 4 bytes, and a msg should be at least
 * 4 bytes below the mailbox size
 */
...
void treasure_error(struct mail_test_ctx *ctx, u8 mail_treasure)
{
	pr_err("--- TREASURE ERROR !! ---\n"
	       ...
	       (unsigned int)mail_treasure, (unsigned int)ctx->mail_size,
	       (unsigned int)ctx->treasure,
	       (unsigned int)ctx->treasure_location,
	       (unsigned int)ctx->old_head, (unsigned int)ctx->old_tail,
	       (unsigned int)ctx->new_head,
	       (unsigned int)ctx->new_tail);
	ctx->error_nb++;
}
		    ...
		    struct mail_test_ctx *ctx, int iteration)
{
	...
	ctx->treasure_location =
		(u16)((mail->body[2] << 8) | (mail->body[1] & 0xff));
	ctx->mail_size = mail->body_size;

	pr_alert("[%d] mail size: %x\n", iteration,
		 (unsigned int)ctx->mail_size);
			     ...
			     struct al5_user *user, unsigned long arg)
{
	...
	err = al5_check_and_send(user, create_set_irq_timer_buffer_msg(
					 &msg_info));
	if (err)
		return err;
